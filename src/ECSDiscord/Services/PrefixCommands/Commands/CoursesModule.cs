...
public class CoursesModule : ModuleBase<SocketCommandContext>
{
    ...
    public CoursesModule(IConfiguration config, ITranslator translator, CourseService courseService,
    ...
    public async Task CreateCategoryAsync(string name, string autoImportPattern, int autoImportPriority)
    {
        ...
        var category = Context.Guild.CategoryChannels
            .DefaultIfEmpty(null)
    ...
    [Summary("Adds a category for course channels.")]
    [RequireUserPermission(GuildPermission.Administrator)]
    public async Task CreateCategoryAsync(SocketCategoryChannel category, string autoImportPattern,
    ...
    public async Task MassImportCourseAsync(string regex)
    {
        ...
        var channels = Context.Guild.Channels.Where(x => pattern.IsMatch(x.Name)).ToList();
        foreach (var channel in channels) await _courseService.CreateCourseAsync(channel);
    ...
    public async Task AllCoursesAsync()
    {
        ...
        builder.AddField("Usage",
            ...
            "e.g. `+join comp102 engr101 engr121 cybr171`");
        if (await _enrollments.RequiresVerification(Context.User))
            builder.AddField(":rotating_light:  You are unverified!  :rotating_light:",
                _translator.T("ALLCOURSES_VERIFICATION_REQUIRED"));
        builder.AddField("100-Level", createCourseBlock(level100Courses));
        builder.AddField("200-Level", createCourseBlock(level200Courses));
        builder.AddField("300-Level", createCourseBlock(level300Courses));
        builder.AddField("400-Level", createCourseBlock(level400Courses));
        builder.AddField("Other", createCourseBlock(otherCourses));
    ...
    {
        var patterns = await _courseService.GetAutoCreatePatternsAsync();
    ...
    {
        var aliases = await _courseService.GetAllAliasesAsync();
        var sb = new StringBuilder();
        sb.Append("```");
