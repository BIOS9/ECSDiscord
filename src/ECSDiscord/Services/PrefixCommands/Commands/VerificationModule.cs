...
public class VerificationModule : ModuleBase<SocketCommandContext>
{
    ...
    public async Task VerifyAsync(string email)
    {
        ...
        {
            var result = await _verification.FinishVerificationAsync(email, Context.User);
            switch (result)
            {
        ...
        {
            var result = await _verification.StartVerificationAsync(email, Context.User);
            switch (result)
            {
    ...
    {
        var overrides =
            await _storage.Verification.GetAllVerificationOverrides(
                StorageService.VerificationStorage.OverrideType.ROLE);
        ...
        var sb = new StringBuilder("**Role Verification Overrides:**\n```");
        overrides.ForEach(x =>
        {
    ...
    public async Task ListForcedUserVerificationsAsync()
    {
        var overrides =
            await _storage.Verification.GetAllVerificationOverrides(
                StorageService.VerificationStorage.OverrideType.USER);
        if (overrides.Count == 0)
        {
        ...
        var sb = new StringBuilder("**User Verification Overrides:**\n```");
        overrides.ForEach(x =>
        {
    ...
    {
        var verifiedUsers = await _storage.Verification.GetVerifiedUsersCount();
