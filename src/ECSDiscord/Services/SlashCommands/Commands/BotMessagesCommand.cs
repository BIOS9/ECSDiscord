...
public class BotMessagesCommand : ISlashCommand
{
    ...
    public BotMessagesCommand(ServerMessageService serverMessageService)
    {
    ...
    public async Task ExecuteAsync(ISlashCommandInteraction command)
    {
        ...
        {
            case "create":
                await CreateMessageAsync(command);
                break;
            case "delete":
                break;
            case "edit":
    ...
    private async Task CreateMessageAsync(ISlashCommandInteraction command)
    {
        await command.RespondWithModalAsync(BotMessageCreateModal.Build());
    }
    ...
    private async Task ListMessagesAsync(ISlashCommandInteraction command)
    {
        ...
        {
            if (command.ChannelId != command.ChannelId.Value)
            {
                continue;
            }
            
            fields.Add(new EmbedFieldBuilder()
                .WithName($"{message.Name} - {message.ID}")
                .WithValue($"Created by {message.Creator.Mention} **{message.CreatedAt.Humanize()}**\n" +
        ...
        {
            await command.RespondAsync("There are no bot messages in this channel.", ephemeral: true);
        }
    }
}